...
function addTodo() {
    ...
    if (!textToDo || textToDo.trim() === "") {
        console.log("Error: Teks to-do tidak boleh kosong.");
        return;
    }
    ...
    let newTodo = {
            id: generateUniqueId(),
            text: textToDo.trim(),  // Store trimmed version
            isCompleted: false,
        };
...
function markTodoCompleted() {
    ...
    // cek jika daftar kosong
        if (todos.length === 0) {
                return;   // Keluar lebih awal, tidak membutuhkan prompt
        }
...
function runTodoApp() {
    ...
    while (running) {
        ...
        let command = prompt("Enter a command inside the square brackets: ");

        // Add null check and trim
        if (!command) {
